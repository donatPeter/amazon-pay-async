...
async function callApiMethod(action, accessToken) {
  ...
  try {
    ...
    return Promise.reject(await parseApiResponse(err));
  }
}
...
async function callMwsMethod(method, version, params) {
  ...
  try {
    ...
    return Promise.reject(await parseMwsResponse(method, err));
  }
}
...
async function parseMwsResponse(method, response) {
  try {
    ...
    if ((response.headers && response.headers['content-type'] === 'text/xml') || response.error) {
      ...
      const result = await xml2jsPromise(response.body || response.error, { explicitArray: false });
